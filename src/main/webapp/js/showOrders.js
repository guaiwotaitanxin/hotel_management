layui.use(['jquery','layer','table','form','laydate'], function() {
    ...
    function loadOrders(){
        table.render({  //底层使用的依然是jquery中的ajax
            ...
            ,cols: [[ //表头  每一列   field: 'username'为实体对象的属性名  title: '用户名' 表格的列名  sort: true 根据此列数据进行排序
                ...
                ,{field: 'orderNum', title: '订单编号', width:220,align:'center'}
                ,{field: 'customerName', title: '客人姓名', width:120, sort: true,align:'center',templet: '<div>{{d.inRoomInfo.customerName}}</div>'}
                ,{field: 'idcard', title: '身份证号', width:200,align:'center',templet: '<div>{{d.inRoomInfo.idcard}}</div>'}
                ,{field: 'isVip', title: 'VIP', width: 80,align:'center',templet:'#isVipTpl'}
                ,{field: 'phone', title: '手机号', width: 180, sort: true,align:'center',templet: '<div>{{d.inRoomInfo.phone}}</div>'}
                ,{field: 'createDate', title: '下单时间', width: 220, sort: true,align:'center'}
                ,{field: 'orderMoney', title: '总价', width: 120,align:'center',event: 'setOrderMoney'}
                ,{field: 'remark', title: '备注', width: 220,align:'center',edit: 'text'}
                ,{field: 'orderStatus', title: '状态', width: 120,align:'center',templet:'#orderStatusTpl'}
    ...
    form.on('submit(demo1)', function(data){
        jsonSelOrders = data.field;
        if(jsonSelOrders.queryTimes!=''){
            //通过切割字符串queryTimes，得到查询的时间的数组
            var arrTimes = jsonSelOrders.queryTimes.split(" - ");
            jsonSelOrders['startDate'] = arrTimes[0];
            jsonSelOrders['endDate'] = arrTimes[1];
        }
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'ordersPay'){ //支付
            layer.confirm('真的要支付此订单么？', function(index){
                //去到订单支付页面
            ...
            layer.confirm('真的删除此订单么？',{icon:7}, function(index){
                updOrdersFlag(obj);//向服务端发送删除指令
                layer.close(index);
            });
        }else if(layEvent === 'setOrderMoney'){
            if(data.orderStatus=='0'){
                layer.prompt({
                    formType: 2
                    ,title: '修改 客人姓名 为 ['+ data.inRoomInfo.customerName +'] 的订单总价（最低再打8折）'
                    ,value: data.orderMoney  //原有的订单总价
                }, function(value, index){
                    var minPrice = data.orderMoney*0.8.toFixed(2);
                    if(value>=minPrice){
    ...
    //修改订单总价
    function updOrdersMoney(obj,value) {
        $.ajax({
            type:'POST',
            url:'orders/updByPrimaryKeySelective',
